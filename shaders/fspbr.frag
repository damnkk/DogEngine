...
void main(){
    ...
    if (umat.textureIndices[3]!=k_invalid_index&&umat.textureUseSetting[1]>0){
    ...
    float metallic = umat.mrFactor.x;
    if (umat.textureIndices[1]!=k_invalid_index&&umat.textureUseSetting[2]>0){
        metallic =texture(globalTextures[nonuniformEXT(umat.textureIndices[1])], fragTexCoord).z;
    }
    float ao = 1.0f;
    if (umat.textureIndices[2]!=k_invalid_index){
        ao = texture(globalTextures[nonuniformEXT(umat.textureIndices[2])], fragTexCoord).r;
    }
    ...
    vec3 emission = umat.emissiveFactor;
    if (umat.textureIndices[4]!=k_invalid_index&&umat.textureUseSetting[3]>0){
        emission=vec3(decode_srgb(texture(globalTextures[nonuniformEXT(umat.textureIndices[4])], fragTexCoord).xyz));
    }
    vec3 color=PBR(N, V, L, baseColor.xyz, vec3(2.f), roughness, metallic)+emission*umat.intensity.y;
