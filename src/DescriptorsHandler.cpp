...
Descriptors::Descriptors(VkDevice *device) { m_Device = device; }
...
void Descriptors::CreateDescriptorPools(size_t swapchain_images,
                                        size_t vp_ubo_size,
                                        size_t light_ubo_size,
                                        size_t settings_ubo_size) {
...
void Descriptors::CreateViewProjectionSetLayout() {
  ...
  viewProjectionLayoutBinding.descriptorType =
      VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
  viewProjectionLayoutBinding.descriptorCount = 1;
  viewProjectionLayoutBinding.stageFlags =
  ...
  VkResult result = vkCreateDescriptorSetLayout(
      *m_Device, &layoutCreateInfo, nullptr, &m_ViewProjectionLayout);

  if (result != VK_SUCCESS)
    throw std::runtime_error(
        "Failed to create the View-Projection Descriptor Set Layout");
}
...
void Descriptors::CreateTextureSetLayout() {
  ...
  VkResult result = vkCreateDescriptorSetLayout(
      *m_Device, &textureLayoutCreateInfo, nullptr, &m_TextureLayout);
...
void Descriptors::CreateInputSetLayout() {
  ...
  normalInputLayoutBinding.descriptorType =
      VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  ...
  std::vector<VkDescriptorSetLayoutBinding> inputBindings = {
      positionInputLayoutBinding, colourInputLayoutBinding,
      normalInputLayoutBinding};

  VkDescriptorSetLayoutCreateInfo inputLayoutCreateInfo = {};
  inputLayoutCreateInfo.sType =
      VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
  inputLayoutCreateInfo.bindingCount =
      static_cast<uint32_t>(inputBindings.size());
  inputLayoutCreateInfo.pBindings = inputBindings.data();

  // Create Descriptor Set Layout
  VkResult result = vkCreateDescriptorSetLayout(
      *m_Device, &inputLayoutCreateInfo, nullptr, &m_InputLayout);

  if (result != VK_SUCCESS) {
    throw std::runtime_error(
        "Failed to create the Input Descriptor Set Layout!");
  }
}
...
void Descriptors::CreateLightSetLayout() {
  ...
  VkResult result = vkCreateDescriptorSetLayout(*m_Device, &layout_info,
                                                nullptr, &m_LightLayout);

  if (result != VK_SUCCESS)
    throw std::runtime_error(
        "Failed to create the Light Descriptor Set Layout");
}
...
void Descriptors::CreateSettingsSetLayout() {
  ...
  VkResult result = vkCreateDescriptorSetLayout(*m_Device, &layout_info,
                                                nullptr, &m_SettingsLayout);

  if (result != VK_SUCCESS)
    throw std::runtime_error(
        "Failed to create the Settings Descriptor Set Layout");
}

void Descriptors::CreateViewProjectionDescriptorSets(
    const std::vector<Buffer> &viewProjectionUBO, size_t dataSize,
    size_t swapchain_size) {
  m_DescriptorSets.resize(swapchain_size);

  std::vector<VkDescriptorSetLayout> setLayouts(swapchain_size,
                                                m_ViewProjectionLayout);

  VkDescriptorSetAllocateInfo allocate_info = {};
  allocate_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
  allocate_info.descriptorPool =
      m_ViewProjectionPool; // Pool per l'allocazione dei Descriptor Set
  allocate_info.descriptorSetCount =
      static_cast<uint32_t>(swapchain_size); // Quanti Descriptor Set allocare
  allocate_info.pSetLayouts =
      setLayouts.data(); // Layout da utilizzare per allocare i set (1:1)

  VkResult res = vkAllocateDescriptorSets(*m_Device, &allocate_info,
                                          m_DescriptorSets.data());

  if (res != VK_SUCCESS) {
    throw std::runtime_error("Failed to allocate the descriptor sets for the "
  ...
  for (size_t i = 0; i < swapchain_size; ++i) {
    ...
    vpSetWrite.descriptorType =
        VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; // Indice nell'array da aggiornare
    vpSetWrite.descriptorCount = 1;        // Numero di descriptor da aggiornare
    vpSetWrite.pBufferInfo =
        &vpBufferInfo; // Informazioni a riguardo dei dati del buffer da bindare

    std::vector<VkWriteDescriptorSet> setWrites = {vpSetWrite};

    vkUpdateDescriptorSets(*m_Device, static_cast<uint32_t>(setWrites.size()),
                           setWrites.data(), 0, nullptr);
  }
}

void Descriptors::CreateInputAttachmentsDescriptorSets(
  ...
  std::vector<VkDescriptorSetLayout> inputSetLayouts(swapchain_size,
                                                     m_InputLayout);
  ...
  input_allocate_info.descriptorSetCount =
      static_cast<uint32_t>(swapchain_size);
  input_allocate_info.pSetLayouts = inputSetLayouts.data();

  VkResult result = vkAllocateDescriptorSets(*m_Device, &input_allocate_info,
                                             m_InputDescriptorSets.data());
  if (result != VK_SUCCESS) {
    throw std::runtime_error(
  ...
  for (size_t i = 0; i < swapchain_size; i++) {
    ...
    positionImageInfo.imageView = position_buffer[i].textureImageView;
    ...
    colourImageInfo.imageView = color_buffer[i].textureImageView;
    colourImageInfo.sampler = color_buffer[i].sampler;

    VkDescriptorImageInfo normalImageInfo = {};
    normalImageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    normalImageInfo.imageView = normal_buffer[i].textureImageView;
    ...
    std::vector<VkWriteDescriptorSet> setWrites = {positionWrite, colourWrite,
                                                   normalWrite};
...
void Descriptors::UpdateInputAttachmentsDescriptorSets(
    size_t swapchain_size, const std::vector<Texture> &position_buffer,
    const std::vector<Texture> &color_buffer,
    const std::vector<Texture> &normal_buffer) {
...
void Descriptors::CreateLightDescriptorSets(std::vector<Buffer> &ubo_light,
                                            size_t data_size,
                                            size_t swapchain_images) {
  m_LightDescriptorSets.resize(swapchain_images);

  std::vector<VkDescriptorSetLayout> desc_set_layouts(swapchain_images,
                                                      m_LightLayout);
  ...
  light_allocate_info.descriptorSetCount =
      static_cast<uint32_t>(swapchain_images);
  light_allocate_info.pSetLayouts = desc_set_layouts.data();

  VkResult result = vkAllocateDescriptorSets(*m_Device, &light_allocate_info,
                                             m_LightDescriptorSets.data());

  if (result != VK_SUCCESS) {
    throw std::runtime_error(
        "Failed to allocate Light Attachment Descriptor Sets!");
  }
  ...
  for (size_t i = 0; i < swapchain_images; i++) {
    ...
    vkUpdateDescriptorSets(*m_Device, static_cast<uint32_t>(set_writes.size()),
                           set_writes.data(), 0, nullptr);
  }
}

void Descriptors::CreateSettingsDescriptorSets(
    std::vector<Buffer> &ubo_settings, size_t data_size,
    size_t swapchain_images) {
  ...
  settings_allocate_info.descriptorSetCount =
      static_cast<uint32_t>(swapchain_images);
  settings_allocate_info.pSetLayouts = desc_set_layouts.data();

  VkResult result = vkAllocateDescriptorSets(*m_Device, &settings_allocate_info,
                                             m_SettingsDescriptorSets.data());

  if (result != VK_SUCCESS) {
    throw std::runtime_error(
        "Failed to allocate Settings Attachment Descriptor Sets!");
  }
...
VkDescriptorPool &Descriptors::GetVpPool() { return m_ViewProjectionPool; }

VkDescriptorPool &Descriptors::GetImguiDescriptorPool() {
  return m_ImguiDescriptorPool;
}

VkDescriptorPool &Descriptors::GetTexturePool() { return m_TexturePool; }

VkDescriptorPool &Descriptors::GetInputPool() { return m_InputPool; }

VkDescriptorPool &Descriptors::GetLightPool() { return m_LightPool; }

VkDescriptorPool &Descriptors::GetSettingsPool() { return m_SettingsPool; }
...
void Descriptors::CreateViewProjectionPool(size_t numOfSwapImgs,
                                           size_t UBOsize) {
  VkDescriptorPoolSize vpPoolSize = {};
  vpPoolSize.type =
      VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; // Che tipo di descriptor contiene (non
                                         // è un descriptor set ma sono quelli
                                         // presenti negli shaders)
  vpPoolSize.descriptorCount =
      static_cast<uint32_t>(UBOsize); // Quanti descriptor

  // MODEL (DYNAMIC UBO)
  /*VkDescriptorPoolSize modelPoolSize = {};
  modelPoolSize.type			  =
  VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC; modelPoolSize.descriptorCount =
  static_cast<uint32_t>(m_modelDynamicUBO.size()); */
  ...
  poolCreateInfo.maxSets = static_cast<uint32_t>(
      numOfSwapImgs); // un descriptor set per ogni commandbuffer/swapimage
  poolCreateInfo.poolSizeCount =
      static_cast<uint32_t>(descriptorPoolSizes.size());  // Quante pool
  poolCreateInfo.pPoolSizes = descriptorPoolSizes.data(); // Array di pool size

  VkResult result = vkCreateDescriptorPool(*m_Device, &poolCreateInfo, nullptr,
                                           &m_ViewProjectionPool);
...
void Descriptors::CreateTexturePool() {
  ...
  samplerPoolSize.descriptorCount = 2048;

  VkDescriptorPoolCreateInfo samplerPoolCreateInfo = {};
  samplerPoolCreateInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
  samplerPoolCreateInfo.maxSets = 2048;
  samplerPoolCreateInfo.poolSizeCount = 1;
  samplerPoolCreateInfo.pPoolSizes = &samplerPoolSize;

  VkResult result = vkCreateDescriptorPool(*m_Device, &samplerPoolCreateInfo,
                                           nullptr, &m_TexturePool);
...
void Descriptors::CreateImguiPool() {
  ...
  VkResult result = vkCreateDescriptorPool(*m_Device, &imgui_pool, nullptr,
...
void Descriptors::CreateInputAttachmentsPool(size_t numOfSwapImgs) {
  ...
  std::vector<VkDescriptorPoolSize> pool_sizes = {
      position_pool_size, color_pool_size, normal_pool_size};
  ...
  VkResult result = vkCreateDescriptorPool(*m_Device, &pool_create_info,
                                           nullptr, &m_InputPool);
  if (result != VK_SUCCESS) {
...
void Descriptors::CreateLightPool(size_t swapchain_images, size_t ubo_size) {
  ...
  VkResult result =
      vkCreateDescriptorPool(*m_Device, &pool_info, nullptr, &m_LightPool);
...
void Descriptors::CreateSettingsPool(size_t swapchain_images, size_t ubo_size) {
  ...
  VkResult result =
      vkCreateDescriptorPool(*m_Device, &pool_info, nullptr, &m_SettingsPool);
