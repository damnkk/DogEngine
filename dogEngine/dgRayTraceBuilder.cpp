...
void RayTracingBuilder::setup(DeviceContext* context) {
  m_context = context;
}
...
void RayTracingBuilder::buildBlas(const std::vector<BlasInput>& input, VkBuildAccelerationStructureFlagsKHR flags) {
  ...
  for (u32 idx = 0; idx < nbBlas; ++idx) {
    ...
    vkGetAccelerationStructureBuildSizesKHR(m_context->m_logicDevice, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &buildAs[idx].buildInfo, maxPrimCount.data(), &buildAs[idx].sizeInfo);
  ...
  scratchBufferInfo.reset().setUsageSize(VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, maxScrachSize).setDeviceOnly(false).setName("blasScratchBufferInfo");
  ...
  VkDeviceAddress scrachAddress = vkGetBufferDeviceAddressKHR(m_context->m_logicDevice, &bufferAddInfo);
...
void RayTracingBuilder::cmdCreateBlas(CommandBuffer* cmd, std::vector<u32> indices, std::vector<BuildAccelerationStructure>& buildAs, VkDeviceAddress scratchAddress, VkQueryPool queryPool) {
  if (queryPool) {
  ...
  for (const auto& idx : indices) {
    ...
    bufferInfo.reset().setUsageSize(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, createInfo.size);
    ...
    vkCreateAccelerationStructureKHR(m_context->m_logicDevice, &createInfo, nullptr, &buildAs[idx].as.accel);
    DebugMessanger::GetInstance()->setObjectName((uint64_t) buildAs[idx].as.accel, std::string("blas") + std::to_string(idx), VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR);
    ...
    vkCmdBuildAccelerationStructuresKHR(cmd->m_commandBuffer, 1, &buildAs[idx].buildInfo, &buildAs[idx].rangeInfo);
    VkMemoryBarrier barrier{VK_STRUCTURE_TYPE_MEMORY_BARRIER};
    barrier.srcAccessMask = VK_ACCESS_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;
    barrier.dstAccessMask = VK_ACCESS_ACCELERATION_STRUCTURE_READ_BIT_KHR;
    vkCmdPipelineBarrier(cmd->m_commandBuffer, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR, 0, 1, &barrier, 0, nullptr, 0, nullptr);
    if (queryPool) {
      vkCmdWriteAccelerationStructuresPropertiesKHR(cmd->m_commandBuffer, 1, &buildAs[idx].buildInfo.dstAccelerationStructure, VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR, queryPool, queryCnt++);
    }
  }
}

void RayTracingBuilder::cmdCompactBlas(CommandBuffer* cmd, std::vector<u32> indices, std::vector<BuildAccelerationStructure>& buildAs, VkQueryPool queryPool) {
  u32                       queryCnt = 0;
  std::vector<VkDeviceSize> compactSizes(static_cast<u32>(indices.size()));
  vkGetQueryPoolResults(m_context->m_logicDevice, queryPool, 0, (u32) compactSizes.size(), compactSizes.size() * sizeof(VkDeviceSize), compactSizes.data(), sizeof(VkDeviceSize), VK_QUERY_RESULT_WAIT_BIT);
  for (auto idx : indices) {
    ...
    BufferCreateInfo bufferInfo{};
    bufferInfo.reset().setUsageSize(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, asCreateInfo.size);
    buildAs[idx].as.buffer = m_context->createBuffer(bufferInfo);
    Buffer* buffer = m_context->accessBuffer(buildAs[idx].as.buffer);
    asCreateInfo.buffer = buffer->m_buffer;
    vkCreateAccelerationStructureKHR(m_context->m_logicDevice, &asCreateInfo, nullptr, &buildAs[idx].as.accel);
    DebugMessanger::GetInstance()->setObjectName((uint64_t) buildAs[idx].as.accel, std::string("blas") + std::to_string(idx), VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR);
...
void RayTracingBuilder::destroyNonCompacted(std::vector<u32> indices, std::vector<BuildAccelerationStructure>& buildAs) {
  for (auto& i : indices) {
    vkDestroyAccelerationStructureKHR(m_context->m_logicDevice, buildAs[i].cleanupAs.accel, nullptr);
...
void RayTracingBuilder::cmdCreateTlas(CommandBuffer* cmd, u32 instanceCount, VkDeviceAddress instBufferAddr, BufferHandle& scratchBufferHandle, VkBuildAccelerationStructureFlagsKHR flags, bool update, bool motion) {
  ...
  vkGetAccelerationStructureBuildSizesKHR(m_context->m_logicDevice, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &buildInfo, &instanceCount, &sizeInfo);
  if(update==false){
    VkAccelerationStructureCreateInfoKHR createInfo{VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR};
    createInfo.type = VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR;
    createInfo.size = sizeInfo.accelerationStructureSize;
    BufferCreateInfo tlasBufferInfo{};
    tlasBufferInfo.reset().setName("tlasBuffer").setUsageSize(VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR|VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, createInfo.size).setDeviceOnly(true);
    ...
    DebugMessanger::GetInstance()->setObjectName((uint64_t)m_tlas.accel, "tlas", VK_OBJECT_TYPE_ACCELERATION_STRUCTURE_KHR);
  }
  BufferCreateInfo scratchBufferInfo{};
  scratchBufferInfo.reset().setDeviceOnly(true).setName("scratchBuffer").setUsageSize(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT|VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, sizeInfo.buildScratchSize);
  ...
  VkDeviceAddress scratchAddr = vkGetBufferDeviceAddressKHR(m_context->m_logicDevice, &scratchAddrInfo);
