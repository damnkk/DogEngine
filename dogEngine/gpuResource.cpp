...
SamplerCreateInfo& SamplerCreateInfo::set_LodMinMaxBias(float min, float max, float bias) {
  m_minLod = min;
  m_maxLod = max;
  m_LodBias = bias;
  return *this;
...
void Texture::setSampler(std::shared_ptr<DeviceContext> context, dg::SamplerHandle handle) {
  if (handle.index != k_invalid_index) {
    Sampler* samp = context->accessSampler(handle);
    Texture* tex = context->accessTexture(m_handle);
    tex->m_imageInfo.sampler = samp->m_sampler;
    return;
  } else {
    DG_WARN("Set invalid sampler for a texture is not allowed.");
    return;
  }
}
...
TextureCreateInfo& TextureCreateInfo::setFlag(u8 flag) {
  m_flags |= flag;
  return *this;
}
