...
class RayTracingBuilder {
 public:
  ...
  void buildTlas(const std::vector<VkAccelerationStructureInstanceKHR>& instances,
                 VkBuildAccelerationStructureFlagsKHR                   flags =
                     VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR,
                 bool update = false);
  void cmdCreateTlas(CommandBuffer* cmd, u32 instanceCount, VkDeviceAddress instBufferAddr,
                     BufferHandle& scratchBufferHandle, VkBuildAccelerationStructureFlagsKHR flags,
  ...
  void buildTlas(const std::vector<T>&                instances,
    ...
    instanceBuffer.reset()
        .setName("InstanceBuffer")
        .setDeviceOnly(true)
        .setUsageSize(VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT
 ...
 protected:
  ...
  void cmdCreateBlas(CommandBuffer* cmd, std::vector<u32> indices,
                     std::vector<BuildAccelerationStructure>& buildAs,
                     VkDeviceAddress scratchAddress, VkQueryPool queryPool);
  void cmdCompactBlas(CommandBuffer* cmd, std::vector<u32> indices,
                      std::vector<BuildAccelerationStructure>& buildAs, VkQueryPool queryPool);
  void destroyNonCompacted(std::vector<u32>                         indices,
                           std::vector<BuildAccelerationStructure>& buildAs);
