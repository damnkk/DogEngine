...
void Renderer::init(std::shared_ptr<DeviceContext> context) {
  ...
  m_camera->getAspect() =
      (float) m_context->m_swapChainWidth / (float) m_context->m_swapChainHeight;
  ShaderCompiler::init();
...
TextureResource* Renderer::createTexture(TextureCreateInfo& textureInfo) {
  ...
  textureInfo.setMipmapLevel(
      std::min(textureInfo.m_mipmapLevel,
               CalcuMipMap(textureInfo.m_textureExtent.width, textureInfo.m_textureExtent.height)));
  ...
  if (textureInfo.m_mipmapLevel > 1) {
    ...
    sampleInfo.set_min_mag_mip(VK_FILTER_LINEAR, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR)
        .set_LodMinMaxBias(0, (float) textureInfo.m_mipmapLevel, 0)
        .set_address_uvw(VK_SAMPLER_ADDRESS_MODE_REPEAT, VK_SAMPLER_ADDRESS_MODE_REPEAT,
                         VK_SAMPLER_ADDRESS_MODE_REPEAT)
        .set_name(textureInfo.name.c_str());
...
BufferResource* Renderer::createBuffer(BufferCreateInfo& bufferInfo) {
  ...
  if (find != m_resourceCache.buffers.end()) {
    DG_WARN("You are trying to create a buffer which has a same name{} with an existing buffer, "
            "renderer will reuse it",
            bufferInfo.name);
    return find->second;
  }
...
SamplerResource* Renderer::createSampler(SamplerCreateInfo& samplerInfo) {
  ...
  if (findRes != m_resourceCache.samplers.end()) {
    DG_INFO("There is a Sampler with the same name:{} in resourceCache, renderer will reuse it",
            samplerInfo.name);
    return findRes->second;
  }
...
std::shared_ptr<Program> Renderer::createProgram(const std::string&    name,
                                                 ProgramPassCreateInfo progPassCI) {
...
void Renderer::destroyBuffer(BufferResource* bufRes) {
  ...
  if (it != m_resourceCache.buffers.end()) { m_resourceCache.buffers.erase(it); }
...
void Renderer::destroyTexture(TextureResource* texRes) {
  ...
  if (it != m_resourceCache.textures.end()) { m_resourceCache.textures.erase(it); }
...
void Renderer::destroySampler(SamplerResource* samplerRes) {
  ...
  if (it != m_resourceCache.samplers.end()) { m_resourceCache.samplers.erase(it); }
...
void Renderer::destroyMaterial(Material* material) {
  ...
  if (it != m_resourceCache.materials.end()) { m_resourceCache.materials.erase(it); }
...
void Renderer::present() { m_context->present(); }
...
void Renderer::executeScene() {
  ...
  m_renderObjects.insert(m_renderObjects.end(), m_resourceLoader->getRenderObjects().begin(),
                         m_resourceLoader->getRenderObjects().end());
}
...
void Renderer::drawScene() {
  ...
  ViewPort viewPort{
      .rect{.x = 0.0f, .y = 0.0f, .width = (float) scissor.width, .height = (float) scissor.height},
      .min_depth = 0.0f,
      .max_depth = 1.0f};
  ...
  for (int i = 0; i < m_renderObjects.size(); ++i) {
    ...
    um.modelMatrix = m_resourceLoader->getSceneGraph()->getGlobalTransformsFromIdx(
        currRenderObject.transformIndex);
...
Material* Renderer::getDefaultMaterial() { return m_defaultMaterial; }
...
TextureHandle Renderer::upLoadTextureToGPU(std::string& texPath, TextureCreateInfo& texInfo) {
  ...
  if (texPath.substr(found + 1) == "ktx") {
    ...
    ktxResult   res = ktxTexture_CreateFromNamedFile(
        texPath.c_str(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &ktxTexture);
    DGASSERT(res == KTX_SUCCESS);
    TextureCreateInfo texCI{};
    texCI.setData(ktxTexture)
        .setName("skyTexture")
        .setFormat(VK_FORMAT_R16G16B16A16_SFLOAT)
        .setFlag(TextureFlags::Mask::Default_mask)
        .setMipmapLevel(ktxTexture->numLevels)
  ...
  if (resource) { return resource->handle; }
  int      imgWidth, imgHeight, imgChannel;
  stbi_uc* ptr = stbi_load(texPath.c_str(), &imgWidth, &imgHeight, &imgChannel, 4);
  texInfo.setName(texName.c_str())
      .setData(ptr)
      .setExtent({(u32) imgWidth, (u32) imgHeight, 1})
      .setMipmapLevel(std::min(texInfo.m_mipmapLevel, CalcuMipMap(imgWidth, imgHeight)));
  TextureHandle texHandle = createTexture(texInfo)->handle;
  if (texInfo.m_mipmapLevel > 1) {
    SamplerCreateInfo samplerCI{};
    samplerCI.set_name(texName.c_str())
        .set_min_mag_mip(VK_FILTER_LINEAR, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR)
        .set_LodMinMaxBias(0.0f, (float) (texInfo.m_mipmapLevel), 0.0f);
...
void Renderer::addImageBarrier(VkCommandBuffer cmdBuffer, Texture* texture, ResourceState oldState,
                               ResourceState newState, u32 baseMipLevel, u32 mipCount,
                               bool isDepth) {
...
void Renderer::executeSkyBox() {
  ...
  bcinfo.reset()
      .setName("skyBoxMaterialUniformBuffer")
      .setDeviceOnly(false)
  ...
  skyLayoutInfo.reset()
      .addBinding({VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, 1, "globalUniform"})
  ...
  descInfo.reset()
      .setName("skyDesc")
      .setLayout(skybox.materialPtr->program->passes[0].descriptorSetLayout[0])
      .buffer(skybox.globalUniform, 0)
      .buffer(skybox.materialUniform, 1);
...
void Renderer::destroySkyBox() {
  ...
  for (auto& i : rj.descriptors) { m_context->DestroyDescriptorSet(i); }
}
