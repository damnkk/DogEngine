...
static void vulkanCreateTexture(DeviceContext* context, const TextureCreateInfo& tc, TextureHandle handle, Texture* texture){
    ...
    imgCreateInfo.usage |= tc.m_imageUsage;
...
BufferHandle DeviceContext::createBuffer( BufferCreateInfo& bufferInfo){
    ...
    if(bufferInfo.m_sourceData){
        ...
        if(!bufferInfo.m_deviceOnly){
            ...
            stagingBufInfo.setName("staging buffer").setData(bufferInfo.m_sourceData).setDeviceOnly(false)
            .setUsageSize(VK_BUFFER_USAGE_TRANSFER_SRC_BIT|VK_BUFFER_USAGE_TRANSFER_DST_BIT,bufferInfo.m_bufferSize);
...
TextureHandle   DeviceContext::createTexture(TextureCreateInfo& tc){
    ...
    if(tc.m_sourceData){
        ...
        VkDeviceSize imageSize = tc.m_textureExtent.width*tc.m_textureExtent.height*4;
        ...
        DGASSERT(vmaCreateBuffer(m_vma, &stagingBufferInfo, &allocCreateInfo, &stagingBuffer, &stagingAllocation , &vmaInfo)==VK_SUCCESS);
        void* data;
        vmaMapMemory(m_vma, stagingAllocation, &data);
        memcpy(data,tc.m_sourceData,imageSize);
        vmaUnmapMemory(m_vma, stagingAllocation);

        VkCommandBufferBeginInfo cmdBeginInfo{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
        cmdBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        CommandBuffer* tempCommandBuffer = getInstantCommandBuffer();
        ...
        bufferImagecp.imageExtent = tc.m_textureExtent;
...
void DeviceContext::init(const ContextCreateInfo& DeviceInfo){
    ...
    TextureCreateInfo tc{nullptr,{m_swapChainWidth,m_swapChainHeight,1},1,VK_FORMAT_D32_SFLOAT,TextureType::Enum::Texture2D,"Depth_Texture",VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT};
